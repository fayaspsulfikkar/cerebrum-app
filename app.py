import os, time, json, sqlite3, datetime, io, csv
import numpy as np, torch
import matplotlib; matplotlib.use('Agg')
import matplotlib.pyplot as plt
from matplotlib.colors import LinearSegmentedColormap
from functools import wraps
from flask import Flask, request, render_template, jsonify, g, redirect, session, url_for, send_file
...
def init_db():
    ...
    ''')
    for col in ['oauth_provider','oauth_id','name']:
        try: db.execute(f'ALTER TABLE users ADD COLUMN {col} TEXT')
        except: pass
    for col in ['category','timeline_path','radar_path']:
        try: db.execute(f'ALTER TABLE analyses ADD COLUMN {col} TEXT')
        except: pass
    db.close()
...
def signup():
    ...
    db.execute('INSERT INTO users (email,password_hash) VALUES (?,?)',(e,generate_password_hash(p)))
    db.commit()
    u=db.execute('SELECT id FROM users WHERE email=?',(e,)).fetchone()
    return jsonify({'token':create_token(u['id'],e),'email':e})

@app.route('/api/login', methods=['POST'])
def login_api():
    d=request.get_json(); e=(d or{}).get('email','').strip().lower(); p=(d or{}).get('password','')
    db=get_db(); u=db.execute('SELECT * FROM users WHERE email=? AND password_hash IS NOT NULL',(e,)).fetchone()
...
def google_callback():
    try:
        ...
        db=get_db()
        u=db.execute('SELECT * FROM users WHERE oauth_provider=? AND oauth_id=?',('google',oid)).fetchone()
        if not u:
            u=db.execute('SELECT * FROM users WHERE email=?',(email,)).fetchone()
            if u:
                db.execute('UPDATE users SET oauth_provider=?,oauth_id=?,name=? WHERE id=?',('google',oid,name,u['id']))
            else:
                db.execute('INSERT INTO users (email,oauth_provider,oauth_id,name) VALUES (?,?,?,?)',(email,'google',oid,name))
            db.commit()
            u=db.execute('SELECT * FROM users WHERE email=?',(email,)).fetchone()
...
def gen_timeline_heatmap(preds, ts_id):
    """6-region x time matrix heatmap."""
...
def gen_multiline(preds, ts_id):
    """All-region activation lines over time."""
    fig, ax = plt.subplots(figsize=(12, 3))
    fig.patch.set_facecolor('#0a0a0a'); ax.set_facecolor('#0a0a0a')
...
def analyze():
    ...
    try:
        ...
        db=get_db()
        cur=db.execute('INSERT INTO analyses (user_id,video_name,category,region_scores,heatmap_path,timeline_path,radar_path,strongest) VALUES (?,?,?,?,?,?,?,?)',
            (g.user_id,fn,cat,json.dumps(results),heatmap,timeline,radar,json.dumps(strongest_list)))
        db.commit()

        return jsonify({"id":cur.lastrowid,"regions":results,"strongest":strongest_list,
...
def get_analyses():
    db=get_db()
    rows=db.execute('SELECT * FROM analyses WHERE user_id=? ORDER BY timestamp DESC',(g.user_id,)).fetchall()
...
def get_analysis(aid):
    db=get_db(); r=db.execute('SELECT * FROM analyses WHERE id=? AND user_id=?',(aid,g.user_id)).fetchone()
...
def delete_analysis(aid):
    db=get_db(); r=db.execute('SELECT id FROM analyses WHERE id=? AND user_id=?',(aid,g.user_id)).fetchone()
    if not r: return jsonify({"error":"Not found"}),404
    db.execute('DELETE FROM analyses WHERE id=?',(aid,)); db.commit()
...
def compare_api():
    ...
    db=get_db()
    a1=db.execute('SELECT * FROM analyses WHERE id=? AND user_id=?',(v1,g.user_id)).fetchone()
    a2=db.execute('SELECT * FROM analyses WHERE id=? AND user_id=?',(v2,g.user_id)).fetchone()
    if not a1 or not a2: return jsonify({"error":"Not found"}),404
    db.execute('INSERT INTO comparisons (user_id,video1_id,video2_id) VALUES (?,?,?)',(g.user_id,v1,v2)); db.commit()
...
def export_pdf(aid):
    ...
    # Embed radar if available
    radar_file = r.get("radar_path","")
    if radar_file:
...
def get_benchmarks(category):
    b=CATEGORY_BENCHMARKS.get(category, CATEGORY_BENCHMARKS["general"])
    return jsonify(b)
